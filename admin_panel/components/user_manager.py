...
class UserManagerComponent:
    ...
    def _render_access_settings(self):
        ...
        with col2:
            ...
            auto_block_suspicious = st.checkbox(" Авто-блокировка подозрительной активности", value=True)
    ...
    def _remove_upload_permissions(self, selected_rows):
        ...
        if not selected_rows:
            st.warning(" Не выбрано ни одного пользователя")
            return
        ...
        try:
            ...
            if selected_rows and len(selected_rows) > 0:
                ...
                if success_count > 0:
                    ...
                    st.warning(" Не удалось убрать права ни у одного пользователя")
            else:
                st.warning(" Выберите пользователей для изменения прав")
        ...
        try:
            ...
            if selected_rows and len(selected_rows) > 0:
                ...
                if success_count > 0:
                    ...
                    st.warning(" Не удалось выдать права ни одному пользователю")
            else:
                st.warning(" Выберите пользователей для изменения прав")
    ...
    def _render_permission_requests_list(self):
                        ...
                        if request['status'] == 'pending':
                            ...
                            with col_b:
                                ...
                                if not st.session_state.get(reject_key, False):
                                    ...
                                    if st.button(" Подтвердить", key=f"confirm_reject_{request_id}", type="primary"):
                                        ...
                                        st.session_state[reject_key] = False
                                        st.rerun()
                                    if st.button(" Отмена", key=f"cancel_reject_{request_id}"):
                                        st.session_state[reject_key] = False
                                        st.rerun()
                            ...
                            with col_c:
                                ...
                                if not st.session_state.get(delete_key, False):
                                    if st.button(" Удалить", key=f"delete_btn_{request_id}"):
                                        st.session_state[delete_key] = True
                                        st.rerun()
                                else:
                                    # Показываем подтверждение удаления
                                    st.warning(" Удаление запроса необратимо!")
                                    # Кнопки без вложенных колонок
                                    if st.button(" Удалить", key=f"confirm_delete_{request_id}", type="primary"):
                                        self._delete_request(request_id)
                                        st.session_state[delete_key] = False
                                        st.rerun()
                                    if st.button(" Отмена", key=f"cancel_delete_{request_id}"):
                                        st.session_state[delete_key] = False
                                        st.rerun()
                        else:
        ...
        if selected_requests_count > 0:
            ...
            with col3:
                if st.button(" Массовое удаление"):
    ...
    def _batch_process_requests(self, action: str):
        ...
        try:
            ...
            if not selected_requests:
                st.warning(" Выберите запросы для обработки")
                return
    ...
    def _batch_delete_requests(self):
        ...
        try:
            ...
            if not selected_requests:
                st.warning(" Выберите запросы для удаления")
                return
