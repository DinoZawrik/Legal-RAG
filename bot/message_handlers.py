...
from bot.text_formatters import get_enhanced_formatter
...
class MessageHandlers:
    ...
    def __init__(self, bot_instance):
        ...
        self.formatter = get_enhanced_formatter()

    def _fix_telegram_formatting(self, text: str) -> str:
        """
        Подготовка текста для отправки в Telegram (HTML mode).
        Использует централизованный форматтер для консистентности.
        """
    ...
    async def send_formatted_message(self, message: Message, text: str):
        ...
        formatted_text = self._fix_telegram_formatting(text)
        
        # Разбиваем на части, учитывая HTML теги (простая разбивка может разорвать теги)
        # TODO: В идеале нужен умный сплиттер HTML, но пока используем сплиттер по длине
        # с запасом, чтобы минимизировать риск разрыва тегов.
        max_length = 3800 
        
        chunks = self.split_long_message(formatted_text, max_length)
    ...
    def split_long_message(self, text: str, max_length: int) -> list:
        ...
        # Используем сплиттер из форматтера, он уже настроен
        return self.formatter.split_long_message(text, max_length)

    async def _send_single_message(self, message: Message, text: str):
        """Отправляет одно сообщение с использованием HTML парсинга."""
        try:
            await message.answer(text, parse_mode=ParseMode.HTML)
        except TelegramBadRequest as e:
            logger.error(f" Ошибка форматирования Telegram (HTML): {e}")
            # Fallback 1: Попробуем отправить без форматирования
            try:
                # Очищаем теги для plain text
                clean_text = re.sub(r'<[^>]+>', '', text)
                await message.answer(clean_text, parse_mode=None)
            except Exception as e2:
                logger.error(f" Ошибка отправки plain text: {e2}")
                await message.answer("Ошибка отображения сообщения.", parse_mode=None)
        except Exception as e:
            logger.error(f" Критическая ошибка отправки сообщения: {e}")
            await message.answer("Ошибка при отправке ответа.", parse_mode=None)
