...
4. Флаг " Low Confidence" если проверка не прошла
...
class AnswerVerifier:
    ...
    if not result.is_valid:
        print(f" Низкая уверенность: {result.warnings}")
    ```
    """
    ...
    def _verify_citations(
        ...
        # Извлекаем все статьи из исходных чанков (из текста + метаданных)
        source_articles = set()
        article_to_chunks = {}
        ...
        for chunk in source_chunks:
            ...
            # Также извлекаем номер статьи из метаданных чанка
            if isinstance(chunk, dict):
                meta = chunk.get('metadata', {})
                meta_article = str(meta.get('article_number', '') or meta.get('article', '') or '')
                if meta_article:
                    chunk_articles.append(meta_article)

            for article in chunk_articles:
                source_articles.add(article)
                if article not in article_to_chunks:
                    article_to_chunks[article] = []
                article_to_chunks[article].append(chunk_text[:200])
        ...
        for cited_article in cited_articles:
            ...
            if exists:
                ...
                self.logger.warning(
                    f"[CITATION] Article {cited_article} not found in source chunks"
                )

        # Считаем валидным если >= 50% цитат подтверждены (а не все 100%)
        all_valid = valid_count >= len(cited_articles) * 0.5 if cited_articles else True
    ...
    def _calculate_confidence(
        ...
        """
        citation_score = 1.0 if citations_valid else 0.3
    ...
    def format_warning_message(self, result: VerificationResult) -> str:
        ...
        warning_parts = [
            f"\n\n ПРЕДУПРЕЖДЕНИЕ: Низкая уверенность в ответе ({result.confidence:.1%})"
        ]
        ...
        if not result.citations_valid:
            ...
            warning_parts.append(
                f"\n Ответ ссылается на статьи, которые отсутствуют в исходных документах: "
