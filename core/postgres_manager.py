...
def get_ssl_config():
    ...
    if ssl_mode in ('require', 'verify-ca', 'verify-full'):
        ssl_context = ssl.create_default_context(
            cafile=getattr(SETTINGS, 'POSTGRES_SSL_ROOT_CERT_PATH', None)
        )
        if ssl_mode == 'verify-full':
            ...
            ssl_context.verify_mode = ssl.CERT_REQUIRED
        else:  # require — encrypt but don't verify
            ssl_context.check_hostname = False
                ...
                for index_sql in indexes_sql:
                    try:
                        ...
                        if "already exists" not in str(idx_error).lower():
                            logger.warning(f" Ошибка создания индекса: {idx_error}")
                ...
                try:
                    async with self.pool.acquire() as conn:
                        ...
                        if skip_duplicates and file_hash:
                            ...
                            logger.info(f" Удалено документов: {delete_result.split()[-1] if delete_result else '0'}")
                    ...
                    if ('unique' in error_msg or 'duplicate' in error_msg) and attempt < MAX_RETRIES - 1:
                        logger.warning(f" Обнаружен race condition (попытка {attempt + 1}), повторяем поиск дубликата: {e}")
